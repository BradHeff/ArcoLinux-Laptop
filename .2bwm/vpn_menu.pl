...
my $options = "-width -30 -location 3 -bw 2 -dmenu -i -p \"$title\" -lines 10";
my $menu = "";

sub get_cur_vpn {
	my $cons = capture { system qq{nmcli -t -f name,type,device con | grep vpn | grep wlp3s0}};
...
sub handle_item {
	...
	if(length $selection gt 0){
		if(connected){
			...
			my $icon = "-i messagebox_critical";
			if($res =~ /activated/){
				$icon = "-i messagebox_info";
				system qq{notify-send -u normal $icon "$selection" "Connection successfully activated"};
			} else {
				system qq{notify-send -u normal $icon "$selection" "Connection Failed (check config)"};
			}
